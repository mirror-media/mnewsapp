...
class _ProgramListWidgetState extends State<ProgramListWidget>{
  ...
  Widget _buildContent(ProgramList programList){
    ...
    int start = programList.indexWhere(
            (element) => element.year == _selectedDate.year && element.weekDay == _selectedDate.weekday
    );
    int end = programList.lastIndexWhere(
            (element) => element.year == _selectedDate.year && element.weekDay == _selectedDate.weekday
    );
    ...
    if(end != programList.length - 1){
      ...
      if(_selectedDate.weekday == 7){
        _pickedProgramList.add(
            programList.firstWhere(
                    (element) => element.weekDay == 1 && element.year == _selectedDate.year,
              orElse: (){
        ...
        _pickedProgramList.add(
            programList.firstWhere(
                    (element) => element.weekDay == _selectedDate.weekday + 1 && element.year == _selectedDate.year,
              orElse: (){
                return programList.firstWhere((element) => element.weekDay == _selectedDate.weekday + 1);
              }
            ));
      }
    }
    ...
    return ListView.separated(
        itemBuilder: (context, index){
          return _programListViewItem(_pickedProgramList[index],_pickedProgramList[index+1]);
        },
        separatorBuilder: (context, index){
          return Divider(height: 16.5, thickness: 0.5, color: Colors.black,);
        },
  ...
  Widget _programListViewItem(ProgramListItem today, ProgramListItem tomorrow){
    String startHour =
    (today.startTimeHour < 10) ? '0${today.startTimeHour.toString()}' : today.startTimeHour.toString();
    String startMinute =
    (today.startTimeMinute < 10) ? '0${today.startTimeMinute.toString()}' : today.startTimeMinute.toString();
    String endHour =
    (tomorrow.startTimeHour < 10) ? '0${tomorrow.startTimeHour.toString()}' : tomorrow.startTimeHour.toString();
    String endMinute =
    (tomorrow.startTimeMinute < 10) ? '0${tomorrow.startTimeMinute.toString()}' : tomorrow.startTimeMinute.toString();
    ...
    if(today.txCategory == 'Repeat')
      newOrRepeat = Text('(重播)', style: TextStyle(fontSize: 15, color: Color(0xE5979797)),);
    else
      newOrRepeat = Text('(新播)', style: TextStyle(fontSize: 15),);
    Widget name = Expanded(
      child: Row(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Expanded(
            child: Text(today.programme,
    ...
    return Row(
      children: [
        ...
        name,
        SizedBox(width: 20,),
        Text(today.showClass, style: TextStyle(fontSize: 15, color: Color(0xE5979797)),)
      ],
    );
  }
}